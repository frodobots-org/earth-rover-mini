...
sConfig.EncoderMode = TIM_ENCODERMODE_TI2;//TIM_ENCODERMODE_TI12
...
struct stm32_pulse_encoder_device *stm32_device;
stm32_device = (struct stm32_pulse_encoder_device*) pulse_encoder;
return (rt_int32_t) ((rt_int16_t) __HAL_TIM_GET_COUNTER( &stm32_device->tim_handler )
            + stm32_device->over_under_flowcount * AUTO_RELOAD_VALUE);
}
...
static const struct rt_pulse_encoder_ops _ops = { .init = pulse_encoder_init , .get_count = pulse_encoder_get_count ,
      .clear_count = pulse_encoder_clear_count , .control = pulse_encoder_control , };
...
for ( i = 0; i < sizeof(stm32_pulse_encoder_obj) / sizeof(stm32_pulse_encoder_obj [ 0 ]) ; i++ )
{
	  ...
	  if ( rt_device_pulse_encoder_register ( &stm32_pulse_encoder_obj [ i ].pulse_encoder ,
		          stm32_pulse_encoder_obj [ i ].name , RT_NULL ) != RT_EOK )
	  {
