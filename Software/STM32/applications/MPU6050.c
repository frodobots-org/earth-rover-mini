...
//    if (cur_acc_factor != 0)
//    {
//        data[DATA_AXIS_X] = (data[DATA_AXIS_X] * cur_acc_factor) / LSM6DSL_ACC_MUL;//单位 1000 * g
...
//            ,cur_acc_factor,data[0],data[1],data[2]);
    ...
    *y_data = data[1];
    *z_data = data[2];
//    rt_kprintf("st_acc_x   %d , st_acc_y   %d , st_acc_z   %d \n",data[DATA_AXIS_X],data[DATA_AXIS_Y],data[DATA_AXIS_Z]);
    return 0;
}
...
//    data[0] = ((3.1415926f / 180.0f) * data[0]) / 16.4f;//1000 * rad/s
//    data[1] = ((3.1415926f / 180.0f) * data[1]) / 16.4f;
//    data[2] = ((3.1415926f / 180.0f) * data[2]) / 16.4f;
    *x_data = data[0];
    *y_data = data[1];
    *z_data = data[2];
    return 0;
