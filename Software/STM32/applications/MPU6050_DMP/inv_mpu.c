...
#define i2c_write   MPU6050_iic_write
#define i2c_read    MPU6050_iic_read
#endif
...
u_int8_t mpu_dmp_init(void)
{
    ...
    if(mpu_init()==0)   //初始化MPU6050
    {
        ...
        if(res)return 3;
        res=dmp_load_motion_driver_firmware();      //加载dmp固件 //校验失败1
        if(res)return 4;
        res=dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation));//设置陀螺仪方向 //校验失败2
        if(res)return 5;
...
//返回值:0,正常
//    其他,失败
u_int8_t mpu_dmp_get_data(float *pitch,float *roll,float *yaw, float *quaternion)
{
