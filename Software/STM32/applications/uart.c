...
////            rt_timer_start ( ucp_data );//开启数据传输
//        }
//        if ( ota )
//        {
//
...
//
//            }
//            rx_length = rt_device_read ( serial , 0 , &rx_buffer , 1 );
...
//                    {
//                        //校验成功，接收IMU&磁力计初始数据
//                        IMU_updata.acc_bias[0] = (int16_t)(ring_buffer [ 8 ] << 8) + ring_buffer [ 7 ];
//                        IMU_updata.acc_bias[1] = (int16_t)(ring_buffer [ 10 ] << 8) + ring_buffer [ 9 ];
//                        IMU_updata.acc_bias[2] = (int16_t)(ring_buffer [ 12 ] << 8) + ring_buffer [ 11 ];
//                        IMU_updata.gyro_bias[0] = (int16_t)(ring_buffer [ 14 ] << 8) + ring_buffer [ 13 ];
//                        IMU_updata.gyro_bias[1] = (int16_t)(ring_buffer [ 16 ] << 8) + ring_buffer [ 15 ];
//                        IMU_updata.gyro_bias[2] = (int16_t)(ring_buffer [ 18 ] << 8) + ring_buffer [ 17 ];
//                        IMU_updata.mag_bias[0] = (int16_t)(ring_buffer [ 20 ] << 8) + ring_buffer [ 19 ];
//                        IMU_updata.mag_bias[1] = (int16_t)(ring_buffer [ 22 ] << 8) + ring_buffer [ 21 ];
//                        IMU_updata.mag_bias[2] = (int16_t)(ring_buffer [ 24 ] << 8) + ring_buffer [ 23 ];
...
//}
//
//static void sys_data_updata_timeout ( void *parameter )
//{
//    uart_report_state();//上传底部状态数据(10HZ)
//    rt_timer_start ( ucp_data );
...
//void uart_data_updata_init ( void )
//{
//      ucp_data = rt_timer_create ( "ucp_data" , sys_data_updata_timeout , RT_NULL , 20 , RT_TIMER_FLAG_ONE_SHOT );
//}
