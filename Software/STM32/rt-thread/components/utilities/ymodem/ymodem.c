    ...
    int tm_sec)
{
    ...
    if (i == tm_sec)
    {
        for(int j = 0;j < i;j++)
        {
            rt_kprintf("%x ",*(ctx->buf + j));
        }
        rt_kprintf("SOH接收超时返回！\n",code);
        return -RYM_ERR_TMO;
    }
    ...
    if (i != (data_sz - 1))
    {
        ...
        rt_kprintf("起始帧接收数据长度: %d,长度不足,超时返回！\n",i);
        return -RYM_ERR_DSZ;
    }
    ...
    if (recv_crc != cal_crc)
    {
        rt_kprintf("起始帧校验失败,打印接收数据,返回错误！\n",code);
        for(int i = 0;i < data_sz;i++)
        {
            rt_kprintf("%x ",*(ctx->buf + i));
        }
        return -RYM_ERR_CRC;
    }
    ...
    enum rym_code *code)
{
    ...
    if (i != tsz)
    {
        rt_kprintf("数据包长度：%d,长度不足,返回错误！",i);
        return -RYM_ERR_DSZ;
    }
    /*****************************************/
    rt_kprintf("数据包接收完成，打印！");
    for(int i = 0;i < tsz;i++)
    {
        rt_kprintf("%x ",*(ctx->buf + i));
    }
